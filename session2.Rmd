...
The annotation of repeated elements within plant genomes can help in the interpretation of observed phenotypes and in computational tasks such as whole genome alignment or pan-genome analyses. Here we describe how the nrTEplants library was constructed by integrating previously available repositories and curated annotations from a variety of plants. This non-redundant library was obtained with the aim of obtaining percentages of repeated sequences similar to those in literature reports describing plant genomes. In addition, emphasis was put in reducing the fraction of protein-coding sequences among candidate repeats. We tested it on 20 plant genomes and compared it to the library REdat, used by default in Ensembl Plants, and to the kmer-based Repeat Detector (Red). We find that nrTEplants outperforms REdat on dicots while masking a similar fraction of gene exons. We also find that RepeatMasker produces better repeat annotations with nrTEplants than minimap2. The repeat library, code and documentation can be downloaded from 


```{r repeats_table}
annot.stats <- read.csv(file="test_data/Ensembl_repeats.tsv", sep="\t", comment.char=";")
annot.stats = annot.stats[,c(1,2,6,7,8,10)]
kable(annot.stats,format.args = list(big.mark=","))
```
