...
<!-- 
pecificity, sensitivity, recall, precision,
Roc, falsos neg, cross validation, sobreentrenamiento
...
In this session we will learn about benchmarks, which are experiments designed to measure the performance of algorithms, methodologies or statistical models. In particular, we will focus on classification problems. These are problems where data instances are assigned to a set of classes.  Note that methods that produce numeric scores can often be disguised as classifiers by adding rules that assign scores to classes.

## Definitions

**Data instance:** individual observation within a data class. Exemple: a phenotype measurement on a plant, or from a single leaf.

**Training set:** Set of data used to train a model or algorithm. 

**Validation set:** Fraction of the data is used to check whether the trained model produces sensible output.

**Test set:** Set of data used to measure the performance of a trained model or algorithm.
...
Usually TEs are annotated by alignment to curated libraries of repeated elements such as [REdat](https://pgsb.helmholtz-muenchen.de/plant/recat) [@Nussbaumer2013], where each sequence or element is classified according to the Wicker classification [@Wicker2007]. 
...
More recently, there are other approaches which don't require any curation; instead, these simply identify repeated elements by counting words ($K$-mers) along the genome. An example of such tools is the [Repeat Detector](https://github.com/BioinformaticsToolsmith/Red) (Red) [@Girgis2015].
...
The process of recognition of DNA sequences by proteins involves readout mechanisms, and also stabilizing atomic interactions that do not confer specificty.
...
The  [Structural Classification of Proteins (SCOP)](http://scop.mrc-lmb.cam.ac.uk) systematically groups protein folds in superfamilies, of which some are the most common DNA-binding proteins. The next table shows superfamilies with more than 20 non-redundant complexes in the [Protein Data Bank](http://www.rcsb.org) as of Dec 2020, as annotated in the database [3d-footprint](http://floresta.eead.csic.es/3dfootprint/):

SCOP superfamily | Number of complexes
---------------- | -------------------
Winged helix (WH) | 114
Homeodomain-like (H) | 104
Glucocorticoid-receptor-like (GR) | 43
Restriction endonuclease-like (RE) | 25 
Homing endonuclease (HE) | 36 
p53-like (P53) | 25
Lambda-repressor-like (LR) | 31
...
The available experimental structures of protein-DNA complexes in the PDB support the annotation of interface residues, those involved directly in sequence recognition, within protein families. The database [footprintDB](http://floresta.eead.csic.es/footprintdb) does exactly this.
...
![Information content of structure-derived DNA motifs of proteins belonging to several SCOP superfamilies as of Dec 2020, after @ContrerasMoreira2010](pics/superfam_specificity.png)
...
**Table 1.** Features of some plant genomes in RSAT::Plants, taken from <http://plants.rsat.eu/data/stats>. Each ID concatenates the organism, the assembly version and the source. Most genome IDs add to the end the Ensembl Plants release number. For instance, Arabidopsis_thaliana.TAIR10.46, corresponds to _A.thaliana_ assembly 10 from TAIR <https://www.arabidopsis.org>, annotated in release 46 of Ensembl Plants. “%Ns” are stretches of uncharacterized nucleotides which often connect assembled sequence contigs. “%repeat-masked” segments are sequences with significant similarity to plant repetitive DNA sequences, which are masked. 
...
2. Choose 'Single organism -> Zea_mays.B73RefGen v4.46' for the examples of this protocol (see __Note  1__), which corresponds to Ensembl Plants release 46
...
2. Choose 'Organism -> Zea mays.B73RefGen v4.46' for the examples of this protocol. 
...
10. In the 'Background model' section select Markov order 2 and choose 'Organism-specific -> Zea mays.B73RefGen v4.46'. Press 'GO'.
