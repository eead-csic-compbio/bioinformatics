...
**Example 12: Create a GitHub repo and pushing a local repo** 
  ...
  + Start by creating a new repository, give it a name (ex: demo_repository), let it public and initialize it with a README file. 
  
  + Now we will connect the local repo to the empty remote one that we just created on GitHub.

```{bash, eval=F}

git remote add origin <repository url> #the URL should look something like https://github.com/najlaksouri/demo_repository .git
