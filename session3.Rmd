...
# Introduction
In most of the cases, publishing an open access paper is not enough for the audience to take advantage of it. The real relevance of a scientific study is its reproducibility.
...
Recently, many scientific journals such Nature, PNAS, Plant physiology, etc. are requiring to submit manuscripts with data accessible repositories and code links (Fanelli 2018; Miyakawa 2020).
In this session we will be using a set of tools to develop a reproducible research. 
...
+ Get used to create a Readme file for your project. 
(Readme is a descriptive file containing set of useful information about a project: title, author, data, software used, fundings ...)
...
The basic concept of reproducibility is to run the analysis using coding scripts (R.scripts, makefiles, etc) with detailed documentation for each step.
...
herein we will compare two Rscripts written for the same purpose
...
There are several literate programming tools: [Jupiter notebooks](https://jupyter.org/) for python users and [R Markdown](https://rmarkdown.rstudio.com/) and [knitr](https://yihui.org/knitr/) for R users. They both work in similar way integrations sections of code and sections of text.

In this session, we will be using the Rmarkdown to generate a html report.

R Markdown file, a plain text file that has the extension .Rmd. The rmarkdown package converts this file into a report, most commonly into HTML or Pdf.
...
![](./pics/rmarkdown_launching.png){height=70% width=70% } 
...
6. Insert R code chunk (insert ---> R) with the documented Rscript of section 2.2.1 and click on knit
...
To unstage undesired file,use the following git command: **git reset HEAD filename**
...
Branching allows users to work on different features at the same time without worring about the safety of the master branch
...
The data generated in your project should be archived and published. For instance, genomic data must be deposited and accessioned prior to publication in a [INSDC](http://www.insdc.org/) repository such as the [European Nucleotide Archive](https://www.ebi.ac.uk/ena/browser/home) .Other related resources are the [European Variation Archive](https://www.ebi.ac.uk/eva/) or [BioSamples](https://www.ebi.ac.uk/biosamples/).
