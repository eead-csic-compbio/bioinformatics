...
**Exercice**  
  ...
  5. Add the index.html to the working directory using the **git add** command.
  
```{bash  eval=FALSE}
  git add index.html
  git add --all #To add edited files at the same time
```
  6. Check again the status.
  7. Commit the changes replacing the <commit message> by "adding an empty index file".
  
```{bash  eval=FALSE}
  git commit -m <commit message>
```  
 8. Edit the index.html file pasting these lines then type **git diff**. What does git diff do?
...
```
  9. Run again the git status,add and commit commands
  10. Execute **git log**. What does this command do? Try also **git log --oneline** 

## Undoing the changes with git

We saw that git log displays all the commits done, assigning a unique indentifier (HEAD) to each one.
To undo the changes, we need to use these identifiers

**Exercice**

 1. Use **git chekout HEAD** to recover the empty version of index.html 
