...
In fact, a reproducible work means making all the work materials: raw data, codes, figures, results available to the readers by hosting it on an accessible platform.
...
In this session we will be using a set of tools to perform a reproducible research. 
...
+ Get used to create a Readme file for your project (Readme is a descriptive file containing useful information about a project such as title, author, data, softwares used, fundings ...).
...
**Recommendations for writing codes**
  ...
  -	Write a clear code for each task (cleaning data, aligning reads, differential analysis, plotting ...) using meaningful file names.
  
  -	The Comment need to be informative and easy to understand for non-specialist readers.
  
  -	Outputs should be redirected automatically to their corresponding directories.
...
Make is a unix tool and makefile is a file containing shell instructions to tell make what to do.
...
3. To execute the makefile you need to indent the target-related instructions using `tabs` as they are part of the syntax. Use vim or any text editor to do so.
