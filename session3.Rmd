...
In this session, you'll be introduced to the concept of a version control system (VCS) to maintain a detailed history of the created versions and the modifications done. 
By using a VCS, we can know when the changes were made and who made them. This applies to documents, images, datafiles and source code, which are the bread and butter of research.

# Current standards of reproducible research
...
In fact, a reproducible work means making all the work materials: raw data, codes, figures available to readers by hosting it on an accessible platform (such as a git repository). Recently, scientific journals such Nature, PNAS, Plant physiology, etc are requiring to submit manuscripts with data accessible repositories and code links [@Fanelli2628; @Miyakawa2020].

By doing so, you are

  * Demonstarting to the audiance the reliabilty of your findings
  * Allowing the readers to adopt your described methods and results in their analysis 

In the course of research projects there are several aspects that need to be addressed in order to ensure your work can reproduced by others and to demonstarte the reliablity of your findings. They are summarized in the next subsections.

## Reproducibilty and Open data
...
There are also resources such as [Figshare](https://figshare.com), [Zenodo](https://zenodo.org) or [Dryad](https://datadryad.org), where you can accession and make citable data, documentation, code or preprints, or [COPO](https://bigdata.cgiar.org/blog-post/copo-a-portal-to-describe-store-and-retrieve-plant-data), which will assist you in archiving all your plant data and outputs, whatever the data types.

## Reproducible data analyses

The basic concept for a reproducible analysis is to 

1. Separte the directories for a better organization:

```{bash, eval=F}
project
│    └───README.md (contains general description of the project)
│     
└───data
│   └─── Raw_data
│   └─── processed_data
...
.
```
2. Perform every analysis using well-documented coding scripts with versioned data and softwares.

Herein we present a simple example of a makefile script:

1. Create a directory called demo_project
2. Copy and save the makefile script as get_sequences.mk
3. Try to run the makefile targets using **make -s -f makefilename target**


```{bash, eval=F}
################################################################
## This makefile is to retrieve 6 genes ids and to assign their
## corresponding fasta sequences 
##
##################################################################

MAKEFILE=~/demo_project/get_sequences.mk
MAKE=make -s -f ${MAKEFILE}

## Define parameters
DATA=~/demo_project/sequences
RESLTS=~/demo_project/results
ARAB_IDS=${DATA}/genelist.txt
ARAB_SEQ=${DATA}/arabidopsis_cdna.fasta

dir:
	@echo "Creating sequences and results directories"
	mkdir -p ${DATA}  #the -p falg means create directory a if it doesn't exist
	mkdir -p ${RESLTS}

get-data:
	@echo "getting the data from the github repo"
  wget https://github.com/eead-csic-compbio/bioinformatics/tree/main/test_data/genelist.txt/
	wget https://github.com/eead-csic-compbio/bioinformatics/tree/main/test_data/arabidopsis_cdna.fasta
	
	@echo
	@echo "moving the data to the sequence folder"
	mv genelist.txt ${DATA}
	mv arabidopsis_cdna.fasta ${DATA}

get-ids:
	@echo
	@echo "Generating 10 random genes from the gene list file"
	shuf -n 10 ${ARAB_IDS} -o ${RESLTS}/random_genes.txt
...
makefile, snakemake, nextflow, docker, conda, galaxy
...
## Installation and getting starte with Git
...
```{bash, eval=F}
cd ~/demo_project
echo "AT2G38470" >> genes_list.txt # add gene of A. thaliana
cat genes_list.txt 
...
Mkdir sequences
# Add 2 empty files
touch sequences/prunus.fa arabidopsis.fa
#add and commit
...
**Example 8: Create a GitHub repo and pushing a local repo** 
